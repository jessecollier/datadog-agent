...
func TestConfigsForPod(t *testing.T) {
			...
			want: []integration.Config{
				{
					...
					Instances:     []integration.Data{integration.Data("{\"prometheus_url\":\"http://%%host%%:%%port%%/metrics\",\"namespace\":\"default_ns\",\"metrics\":[\"*\"]}")},
			...
			want: []integration.Config{
				{
					Name:          "openmetrics",
					InitConfig:    integration.Data("{}"),
					Instances:     []integration.Data{integration.Data("{\"prometheus_url\":\"foo/bar\",\"namespace\":\"default_ns\",\"metrics\":[\"*\"]}")},
					Provider:      names.Prometheus,
					Source:        "kubelet:foo-ctr-id",
					ADIdentifiers: []string{"foo-ctr-id"},
				},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !reflect.DeepEqual(configs, tt.want) {
				t.Errorf("PrometheusCheck.matchPod() configs = %v, want %v", configs, tt.want)
			}
		})
	}
}
