...
const (
	...
	openmetricsDefaultNS   = "default_ns" // TODO: make the field optional in the check
	...
	Instances: []*OpenmetricsInstance{
		{
			...
			Namespace: openmetricsDefaultNS, // TODO: make namespace optional
		},
	},
	AD: &ADConfig{
...
func (pc *PrometheusCheck) initInstances() {
	...
	for _, instance := range pc.Instances {
		...
		if instance.Namespace == "" {
			instance.Namespace = openmetricsDefaultNS // TODO: make namespace optional
		}
	}
}
...
func (pc *PrometheusCheck) initAD() {
	if pc.AD == nil {
		pc.AD = &ADConfig{}
	}

	pc.AD.defaultAD()
	pc.AD.setContainersRegex()
}
...
func (ad *ADConfig) setContainersRegex() {
	ad.containersRe = nil
	...
	if err != nil {
		log.Warnf("Ignoring container names regex '%s': %v", regexString, err)
		return
	}
