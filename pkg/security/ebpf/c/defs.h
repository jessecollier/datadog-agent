...
#define SYSCALL_ABI_HOOKx(x,word_size,type,TYPE,prefix,syscall,...) \
    int __attribute__((always_inline)) type##__##sys##syscall(__JOIN(x,__SC_DECL,__VA_ARGS__)); \
    SEC(#type "/" SYSCALL##word_size##_PREFIX #prefix SYSCALL_PREFIX #syscall) \
    int type##__ ##word_size##_##prefix ##sys##syscall(struct pt_regs *ctx) { \
...
#define SYSCALL_COMPAT_KPROBE0(name, ...) SYSCALL_HOOKx(0,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE1(name, ...) SYSCALL_HOOKx(1,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE2(name, ...) SYSCALL_HOOKx(2,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE3(name, ...) SYSCALL_HOOKx(3,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE4(name, ...) SYSCALL_HOOKx(4,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE5(name, ...) SYSCALL_HOOKx(5,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)
#define SYSCALL_COMPAT_KPROBE6(name, ...) SYSCALL_HOOKx(6,kprobe,KPROBE,compat_,_##name,__VA_ARGS__)

#define SYSCALL_COMPAT_KRETPROBE(name, ...) SYSCALL_HOOKx(1,kretprobe,KRETPROBE,compat_,_##name,struct pt_regs*,ctx)
